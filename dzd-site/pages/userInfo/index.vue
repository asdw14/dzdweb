    ...
    <el-tabs tab-position="left" style="height: 200px;height:100%">
      <el-tab-pane label="个人信息">
        <div style="margin-left: 10%">
          ...
          <el-avatar :src="userInfo.avatar" style=""></el-avatar>
          <el-button type="primary">更换<i class="el-icon-upload el-icon--right"></i></el-button>

          <br><br>
          积分{{ userInfo.credit }} <br><br>
          空间大小{{ userInfo.surplus }}/
          {{ userInfo.datasize }}
          下载次数{{ userInfo.quantity }} <br><br>
          性别：{{ userInfo.sex == 1 ? '女' : userInfo.sex == 2 ? '男' : '未设置' }} <br><br>
          年龄：{{ userInfo.age }} <br><br>
        </div>
...
<br><br><br><br>
      ...
      <el-tab-pane label="下载记录" @click="getDownLog" :select="getDownLog">
        <span style="font-size: 20px;margin-left: 5%;color: #1989fa">
          {{ this.sourceList != null ? this.sourceList.length : 0 }}条浏览记录
        </span>
        <el-table :data="sourceList" style="width: 80%; margin-left: 10% ">
          ...
          <el-table-column width="45" fixed="left">
            ...
            <template slot-scope="scope">
              <div v-if="scope.row.originalName !=null">
          ...
          <el-table-column label="下载时间" sortable prop="gmtCreate" width="150">
            <template slot-scope="scope">
              ...
              {{ scope.row.gmtModified == null ? '已删除' : scope.row.gmtModified.substr(0, 16) }}
            </template>
          </el-table-column>
          ...
          <el-table-column label="大小" width="130" fixed="right" sortable prop="fileSize">
            <template slot-scope="scope">
              <div v-if="scope.row.fileSize!=null">
          ...
          <el-table-column fixed="right" width="50">
            ...
            <template slot-scope="scope">
              <div v-if="scope.row.sourceOssUrl!=null">
      ...
      <el-tab-pane label="浏览记录">
        <span style="font-size: 20px;margin-left: 5%;color: #1989fa">
          {{ this.articleViewList != null ? this.articleViewList.length : 0 }}条浏览记录
        </span>

        <el-card class="box-card" shadow="always" v-for="article in articleViewList" :key="article.id"
            ...
            <el-col :span="7" :offset="1">
              <div style="font-size:1px; color: #888888;">
                <i class="el-icon-time"
                   style="margin: 0 10px; font-size: 16px; color: #888888">{{ article.gmtModified.substr(0, 16) }}</i>
              </div>
            </el-col>
          </el-row>
      ...
      <el-tab-pane label="点赞记录">
        <span style="font-size: 20px;margin-left: 5%;color: #1989fa">
          {{ this.articleStarList != null ? this.articleStarList.length : 0 }}条点赞记录
        </span>

        <el-card class="box-card" shadow="always" v-for="article in articleStarList" :key="article.id"
                 style="width: 80%;margin-left: 10%">
          <!-- 标题 -->
          <el-row>
