<template>
  <div>
    <section class="container">
      ...
      <el-button type="primary" size="medium" round="true" @click="onshowUpload(true)"><i class="el-icon-upload el-icon--left"></i>上传</el-button>

      <!--  上传文件  -->

      <el-dialog title="上传后请等待进度条消失，对应文件右边变为绿色的 √ 后再关闭窗口进行其他操作，否则会上传失败" :visible.sync="showUpload" :destroy-on-close=true
        width="40%" v-loading="loading" element-loading-text="拼命上传中" element-loading-spinner="el-icon-loading">

        请输入资源描述：
        <input v-model="sourceName" style="border-radius: 20px;  width: 40%;
        ...
        <el-divider></el-divider>
        <el-upload class="upload-demo" ref="upload" :action="url+sourceName+'/'+price" :on-preview="handlePreview"
          :on-remove="handleRemove" :file-list="fileList" :on-progress="onprogress" :on-success="uploadSuccess"
          :before-upload="beforeUpload" :headers="headers" limit="1" :on-error="onerror" name="file" :auto-upload="false">
    ...
    <el-switch
    style="size: 20px;margin-left: 55%"
      ...
      <!--   查找文本框   -->
      <input v-model="searchObj.sourceName" style="border-radius: 20px;  width: 150px;
            height: 28px;  border-color: #3399ff;
            padding-left: 10px; margin-left: 6%"
        type="text" placeholder="请输入想搜索的个人资源" />
      <!--   查找按钮   -->
      <el-button icon="el-icon-search" circle type="success" size="small" @click="getPublicPageList(1)"></el-button>


      <el-table :data="sourceList" style="width: 100%" :default-sort="{ order: 'descending'}">
      ...
      <el-pagination :current-page="page" :page-size="limit" :total="total" style="padding: 30px 0; text-align: center;"
        layout="total, prev, pager, next, jumper" @current-change="getPublicPageList" />
    </section>
  </div>
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //分页查询公开资源
      getPublicQueryPageList() {
        source.getPublicQueryPageList(this.page, this.limit, this.searchObj).then((response) => {
          this.sourceList = response.data.data.items
          this.total = response.data.data.total
        });
      },
